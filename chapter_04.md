...
Usaremos essa imagem para discorrer sobre pontos de atenção que vale a pena aplicar em sua arquitetura:
...
Caso você venha do DDD, não verá muita novidade nessa camada e nos conceitos. Ela é responsável por encapsular o domínio do negócio. O ponto principal é que essa camada é o core, ou seja, é a razão de fazer a aplicação em si. Isso significa que é nela que se concentram as regras de negócio e isso não deve mudar de acordo com itens externos, como, por exemplo, mudança de banco de dados.
...
Nessa camada se concentram as ações da regra de negócios. Uma maneira de pensar é de que esta seja uma continuação da camada de entidade. Muitas regras que envolvem as entidades ficam muito grandes para caber apenas na entidade, sem falar no clássico problema de responsabilidade única de que tanto falamos no [SOLID](https://en.wikipedia.org/wiki/SOLID).
